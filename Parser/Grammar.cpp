...
void Grammar::readFile(std::string filename)
{
	...
	while (std::getline(infile, line)) {
		...
		while ((pos = line.find(bara)) != std::string::npos) {
			...
			productions[start].push_back(token);
			line.erase(0, pos + bara.length());
		}
		//productions[start[0]].push_back(line);
		productions[start].push_back(line);
	}
}
...
std::string Grammar::prettyPrint()
{
	...
	for (std::map<std::string, std::vector<std::string>>::iterator it = productions.begin(); it != productions.end(); ++it) {
		res = res + it->first + " = ";
...
std::vector<std::string> Grammar::getProdForNonT(std::string q)
{
	return productions[q];
}

std::string Grammar::getInitialState()
{
	return q0;
}
...
bool Grammar::isLeftRecursive()
{
	for (std::map<std::string, std::vector<std::string>>::iterator it = productions.begin(); it != productions.end(); ++it) {
		if (it->first.compare(it->second[0]) == 0) { return true; }
	}
	return false;
}
